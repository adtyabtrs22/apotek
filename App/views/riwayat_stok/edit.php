...
<head>
    ...
    <title>Edit Riwayat Stok - Apotek</title>
...
<body>
    ...
    <div class="content">
        <div class="form-container">
            ...
            <form method="POST" action="<?php echo BASE_URL; ?>/riwayat_stok/edit/<?php echo $data['riwayat_stok']['id_riwayat']; ?>">
                <label for="id_obat">Obat:</label>
                <select name="id_obat" required>
                    <option value="">-- Pilih Obat --</option>
                    <?php foreach ($data['nama_obat'] as $obat): ?>
                        <option value="<?php echo $obat['id_obat']; ?>" <?php echo ($data['riwayat_stok']['id_obat'] == $obat['id_obat']) ? 'selected' : ''; ?>>
                ...
                <input type="date" name="tanggal" value="<?php echo htmlspecialchars($data['riwayat_stok']['tanggal']); ?>" required>

                <label for="jenis_transaksi">Jenis Transaksi:</label>
                <select name="jenis_transaksi" required>
                    <option value="">-- Pilih Jenis Transaksi --</option>
                    <option value="Pengadaan" <?php echo ($data['riwayat_stok']['jenis_transaksi'] == 'Pengadaan') ? 'selected' : ''; ?>>Pengadaan</option>
                    <option value="Distribusi" <?php echo ($data['riwayat_stok']['jenis_transaksi'] == 'Distribusi') ? 'selected' : ''; ?>>Distribusi</option>
                    <option value="Pemeriksaan" <?php echo ($data['riwayat_stok']['jenis_transaksi'] == 'Pemeriksaan') ? 'selected' : ''; ?>>Pemeriksaan</option>
                </select>

                <label for="jumlah">Jumlah:</label>
                <input type="number" name="jumlah" min="0" value="<?php echo htmlspecialchars($data['riwayat_stok']['jumlah']); ?>" required>

                <label for="stok_akhir">Stok Akhir:</label>
                <input type="number" name="stok_akhir" min="0" value="<?php echo htmlspecialchars($data['riwayat_stok']['stok_akhir']); ?>" required>
