...
<body>
    ...
    <center><h2>Pemeriksaan Stok</h2></center>
        <a href="<?php echo BASE_URL; ?>/pemeriksaan_stok/create" class="button">Tambah Pemeriksaan Baru</a>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php foreach($data['pemeriksaan_stok'] as $pemeriksaanstok): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($pemeriksaanstok['id_pemeriksaan']); ?></td>
                        <td><?php echo htmlspecialchars($pemeriksaanstok['tanggal_pemeriksaan']); ?></td>
                        <td><?php echo htmlspecialchars($pemeriksaanstok['nama_user']); ?></td>
                        <td><?php echo htmlspecialchars($pemeriksaanstok['jabatan']); ?></td>
                        <td><?php echo htmlspecialchars($pemeriksaanstok['keterangan']); ?></td>
                        <td>
                            <a href="<?php echo BASE_URL; ?>/pemeriksaan_stok/edit/<?php echo $pemeriksaanstok['id_pemeriksaan']; ?>">Edit</a> |
                            <a href="<?php echo BASE_URL; ?>/pemeriksaan_stok/delete/<?php echo $pemeriksaanstok['id_pemeriksaan']; ?>" 
                               onclick="return confirm('Apakah Anda yakin ingin menghapus pemeriksaan ini?');">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                <?php if(empty($data['pemeriksaan_stok'])): ?>
                    <tr>
                        <td colspan="6">Tidak ada data pemeriksaan stok.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
